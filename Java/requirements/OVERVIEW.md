...
| 015 | Inventory Table Sorting | in-progress | Server-side sorting for the inventory table: `GET /api/items` gains `sortBy` and `sortDir` query params; the Angular frontend sends sort state from Clarity column header clicks to the backend.
| 016 | OpenAPI REST Specification | ready-for-development | Auto-generate OpenAPI 3.0 specification from Spring Boot REST API using springdoc-openapi; expose spec at `/v3/openapi.json` and Swagger UI at `/swagger-ui.html`. |
