...
| 023 | Backend Folder Restructure | ready-for-development | Reorganize into a monorepo with `/backend` and `/frontend` at the same level; move `src/`, `build/`, `gradle/`, `gradlew` files under `backend/`; keep build config at root. | |
