 ...
 *
 * <p>Provides endpoints for listing items, advancing the shop day, retrieving item prices, and
 * projecting future item states.
 */
@RestController
...
public class ShopController {
  ...
  @Operation(
      ...
      description =
          "Retrieve all items currently held in the shop inventory, with optional sorting.")
  @ApiResponse(
      responseCode = "200",
  ...
  public List<ItemDto> getItems(
      @Parameter(description = "Field to sort by: name, sellIn, or quality")
  ...
  @Operation(
      summary = "Advance shop by one day",
      description =
          "Progress the shop forward by one day, applying quality adjustments to all items based on their type and sell-in status.")
  @ApiResponse(
      responseCode = "200",
      description = "Shop advanced successfully, updated inventory returned",
      content =
          @Content(
              mediaType = "application/json",
              schema = @Schema(implementation = ItemDto.class)))
  ...
  @Operation(
      ...
      description = "Retrieve the current market price of a named item.")
  @ApiResponses({
    @ApiResponse(
        responseCode = "200",
  ...
  @Operation(
      summary = "Get item state projection",
      description =
          "Simulate the state of a named item after n days without mutating the live inventory.")
  @ApiResponses({
    @ApiResponse(
        responseCode = "200",
    ...
    @ApiResponse(
        responseCode = "400",
        description = "Invalid days parameter (must be non-negative)"),
    @ApiResponse(responseCode = "404", description = "Item not found")
  })
  public ResponseEntity<ItemDto> getProjection(
      ...
      @Parameter(description = "Number of days to project forward (must be non-negative)")
          @RequestParam
          final int days) {
  ...
  @Operation(
      summary = "Get bulk shop inventory projection",
      description =
          "Simulate the state of all shop items after n days without mutating the live inventory.")
  @ApiResponses({
    @ApiResponse(
        responseCode = "200",
        description = "Bulk projection calculated successfully",
        content =
            @Content(
                mediaType = "application/json",
                schema = @Schema(implementation = ItemDto.class))),
