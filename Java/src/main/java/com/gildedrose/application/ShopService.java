...
/**
 * Application service that orchestrates shop operations. Coordinates between the API layer and
...
public class ShopService {
  ...
  /**
   * Advances the shop by one day, updating all item qualities and persisting the changes. Publishes
   * domain events for item changes.
   *
  ...
  private List<ItemDto> mapToDto(final List<Item> items) {
    return items.stream()
        .map(
            item ->
                new ItemDto(item.name, item.sellIn, item.quality, pricingService.priceFor(item)))
        .toList();
  }

  /**
   * Interface defining the use case for advancing the shop by one day. Implemented by GildedRose.
   */
