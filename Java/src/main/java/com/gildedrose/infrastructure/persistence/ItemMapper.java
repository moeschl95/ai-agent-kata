...
/**
 * Utility class for mapping between {@link Item} (domain model) and {@link ItemEntity} (persistence
 * model). Centralizes conversion logic to maintain separation of concerns.
 */
public final class ItemMapper {
  ...
  /**
   * Converts a domain {@link Item} to a persistence {@link ItemEntity} with the given ID. Used to
   * preserve IDs when updating existing entities.
   *
   * @param item the domain item to convert
   ...
   */
  public static List<ItemEntity> toEntitiesWithIds(
      final List<Item> items, final Map<Item, Long> idMap) {
    return items.stream()
        .map(
            item -> {
