...
/**
 * Initializes the item database with default shop inventory on application startup. Seeds the ITEMS
 * table only if no items exist yet.
 */
@Component
public class ItemDataInitializer {
  ...
  /**
   * Initializes the database with default items when the application is ready. Only seeds if the
   * ITEMS table is currently empty.
   */
  ...
  private void seedDefaultItems() {
    final List<ItemEntity> defaultItems =
        List.of(
