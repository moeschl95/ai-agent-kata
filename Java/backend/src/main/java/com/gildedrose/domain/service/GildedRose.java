...
/**
 * Core domain service for the Gilded Rose shop. Manages quality updates for items and publishes
 * domain events. Implements the use case interface for advancing the day. Maintains backward
 * compatibility with test code by supporting both update signatures.
 */
public class GildedRose implements GildedRoseUseCase {
  ...
  /**
   * Constructs a new GildedRose shop with the given list of items. This constructor is used for
   * testing and does not persist items or publish events.
   *
  ...
  /**
   * Constructs a new GildedRose shop with the given list of items and an event publisher. This
   * constructor is used for testing with event verification.
   *
   * @param items the initial inventory of the shop
  ...
  /**
   * Constructs a new GildedRose shop using a repository port to load and persist items. Events are
   * not published when using this constructor.
   *
  ...
  /**
   * Constructs a new GildedRose shop using a repository port to load and persist items. Publishes
   * domain events when advancing the day.
   *
   * @param repository the repository port for item persistence
   * @param publisher the event publisher used to publish domain events
   */
  public GildedRose(
      final ItemRepositoryPort repository, final ApplicationEventPublisher publisher) {
  ...
  /**
   * Backward-compatible updateQuality() method for test code. Updates the quality of all items in
   * the internal items list. Only works when GildedRose was constructed with List<Item> or
   * List<Item> + publisher. If constructed with a repository, this delegates to the
   * repository-aware updateQuality(List).
   */
  ...
  /**
   * Implementation of {@link GildedRoseUseCase}: advances all items by one day, applying each
   * item's quality-update rules. Publishes domain events for each changed item and a single {@link
   * DayAdvancedEvent} at the end.
   *
