...
class GildedRoseIntegrationTest {
  ...
  @Autowired private ItemRepository jpaRepository;
  ...
  void should_persistItemsAfterUpdateQuality_when_repositoryIsAvailable() {
    ...
    final ItemEntity brie =
        persisted.stream().filter(e -> e.name.equals("Aged Brie")).findFirst().orElseThrow();
