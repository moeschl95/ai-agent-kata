...
/**
 * Tests that {@link GildedRose#updateQuality()} publishes the correct domain events via the Spring
 * event bus when an {@link ApplicationEventPublisher} is wired.
 */
class GildedRoseEventTest {
  ...
  void should_publishExactlyOneDayAdvancedEvent_when_updateQualityIsCalled() {
    // Arrange
    final GildedRose shop =
        new GildedRose(
            List.of(new Item("Normal Item", 5, 10), new Item("Aged Brie", 3, 20)), publisher);
    ...
    final long dayEventCount =
        captor.getAllValues().stream().filter(e -> e instanceof DayAdvancedEvent).count();
  ...
  void should_includeFinalItemsInDayAdvancedEvent_when_updateQualityIsCalled() {
    ...
    final DayAdvancedEvent dayEvent =
  ...
  void should_publishItemExpiredEvent_when_itemQualityReachesZero() {
    ...
    final ArgumentCaptor<Object> captor = ArgumentCaptor.forClass(Object.class);
    verify(publisher, atLeastOnce()).publishEvent(captor.capture());
    final List<ItemExpiredEvent> expiredEvents =
        captor.getAllValues().stream()
  ...
  void should_publishItemExpiredEvent_for_eachItemThatExpires() {
    ...
    final GildedRose shop =
        new GildedRose(List.of(new Item("Item A", 5, 1), new Item("Item B", 3, 1)), publisher);
  ...
  void should_publishItemQualityChangedEvent_when_qualityChangesButNotToZero() {
    ...
    final ArgumentCaptor<Object> captor = ArgumentCaptor.forClass(Object.class);
    verify(publisher, atLeastOnce()).publishEvent(captor.capture());
    final List<ItemQualityChangedEvent> changedEvents =
        captor.getAllValues().stream()
  ...
  void should_notPublishItemQualityChangedEvent_when_qualityDoesNotChange() {
    ...
    final GildedRose shop =
        new GildedRose(List.of(new Item("Sulfuras, Hand of Ragnaros", 0, 80)), publisher);
  ...
  void should_notPublishItemQualityChangedEvent_when_qualityChangesToZero() {
    ...
    final ArgumentCaptor<Object> captor = ArgumentCaptor.forClass(Object.class);
    verify(publisher, atLeastOnce()).publishEvent(captor.capture());
    final long changedCount =
        captor.getAllValues().stream().filter(e -> e instanceof ItemQualityChangedEvent).count();
